...
public class touchenemy : MonoBehaviour {
	...
	void Update () {
		if (Time.time > nextFire) {
						...
						if (randomNumber == 0) {
								enemy2.transform.position = new Vector3 (1, 0, 20);
				...
				Destroy(clone_en, 1.4f);
				Destroy(clone_en2, 1.4f);
				Destroy(clone_en3, 1.4f);
						}
						else if (randomNumber == 1) {
								enemy2.transform.position = new Vector3 (3, 0, 20);
				nextFire = Time.time + shootrate;
				...
				Destroy(clone_en3, 1.4f);
								...
								enemy2.transform.position = new Vector3 (5, 0, 20);
				nextFire = Time.time + shootrate;
